...
SELECT 
	...
	SUM(DATEDIFF(l.login_time, u.reg_time)=1) AS '次日留存用户数',           # 如果是相距1 那么就返回true 求和 就可以得到有多少人在次日登陆
    SUM(DATEDIFF(l.login_time, u.reg_time)=3) AS '三日留存用户数',
    SUM(DATEDIFF(l.login_time, u.reg_time)=7) AS '七日留存用户数',
    CONCAT(ROUND(SUM(DATEDIFF(l.login_time, u.reg_time)=1) / COUNT(DISTINCT u.user_id) * 100, 2), '%') AS '次日留存率',
    CONCAT(ROUND(SUM(DATEDIFF(l.login_time, u.reg_time)=3) / COUNT(DISTINCT u.user_id) * 100, 2), '%') AS '三日留存率',
    CONCAT(ROUND(SUM(DATEDIFF(l.login_time, u.reg_time)=7) / COUNT(DISTINCT u.user_id) * 100, 2), '%') AS '七日留存率'
FROM user_info u
